...
class FormController extends Controller
{
    ...
    private function handleFileUploads(Request $request, Document $document)
    {
        ...
        if ($request->hasFile('file_path_pdf')) {
            $file = $request->file('file_path_pdf');
            $path = $file->store('documents', 'public');
            $fileData['file_path_pdf'] = $path; // Store basename
            $file->getClientOriginalName(); // Store original file name
        }
    
        if ($request->hasFile('file_path_doc')) {
            $file = $request->file('file_path_doc');
            $path = $file->store('documents', 'public');
            $fileData['file_path_doc'] = $path; // Store basename
             $file->getClientOriginalName(); // Store original file name
        }
    ...
    public function store(Request $request)
    {
        ...
        $document->fill([
            ...
            'original_file_name_pdf' => $fileData['original_file_name_pdf'] ?? $document->original_file_name_pdf,
            'original_file_name_doc' => $fileData['original_file_name_doc'] ?? $document->original_file_name_doc,
